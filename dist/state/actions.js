import { TODAY_ISO, addDays, parseLocalISODate, startOfWeekMonday, toLocalISODate, } from "../utils/date.js";
import { getDateFromInboxDestination, getInboxSeedTasks } from "./selectors.js";
import { buildProjectId, cloneProjectDraft, createProjectSetupState, parseProjectDeadlineInput, submitProjectSetupMessage, } from "./project-bay.js";
...
export function updateTaskField(state, taskId, field, value) {
    state.tasks = state.tasks.map((task) => task.id === taskId ? { ...task, [field]: value } : task);
}
...
export function saveTaskCardEdit(state) {
    if (!state.editingTaskId || !state.editingTaskDraft)
        return;
...
export function updateSubtaskTitle(state, taskId, subtaskId, value) {
    state.tasks = state.tasks.map((task) => {
        ...
        return {
            ...task,
            subtasks: task.subtasks.map((subtask) => subtask.id === subtaskId ? { ...subtask, title: value } : subtask),
        };
    });
}
...
export function resetInbox(state, initialTasks) {
    cancelTaskCardEdit(state);
    const preservedTasks = state.tasks.filter((task) => task.status !== "todo" || task.dueAt || task.projectId);
