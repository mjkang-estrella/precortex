...
export function parseProjectDeadlineInput(value) {
    ...
    if (text === "next week")
        return toLocalISODate(addDays(TODAY, 7));
...
export function generateProjectDraft(answers) {
    ...
    const summary = [
        ...
        ensureSentence(answers.motivation, "It needs a sharper plan before execution starts."),
        ensureSentence(answers.success
            ? `Success looks like ${answers.success}`
            : "Success needs to be defined early so execution does not drift", "Success needs to be defined early so execution does not drift."),
        ensureSentence(answers.constraints
            ? `Constraints to respect: ${answers.constraints}`
            : answers.risks
                ? `Main risk to watch: ${answers.risks}`
                : "The first pass should surface the main blockers early", "The first pass should surface the main blockers early."),
...
export function submitProjectSetupMessage(projectSetup, rawText) {
    ...
    if (nextState.promptKey === "name") {
        if (text.length < 3) {
            nextState.messages.push(createMessage("assistant", "give me a slightly clearer project name so i can anchor the plan."));
            return nextState;
        }
        nextState.answers.name = text;
        nextState.promptKey = "deadline";
        nextState.messages.push(createMessage("assistant", `good. when is ${text.toLowerCase()} due? you can say something like "2026-04-30", "tomorrow", or "in 10 days".`));
        return nextState;
    }
    if (nextState.promptKey === "deadline") {
        const deadline = parseProjectDeadlineInput(text);
        if (!deadline || isPastDate(deadline)) {
            nextState.messages.push(createMessage("assistant", "i need a real future deadline to shape the plan. try a date like 2026-04-30 or a phrase like tomorrow."));
            return nextState;
        }
        nextState.answers.deadline = deadline;
        nextState.promptKey = "outcome";
        nextState.messages.push(createMessage("assistant", QUESTION_COPY.outcome));
        return nextState;
    }
    ...
    if (hasDraftInputs(nextState.answers) && countFilledAnswers(nextState.answers) >= 3) {
        ...
        nextState.messages.push(createMessage("assistant", "i have enough to draft the plan. review it, edit anything that feels off, then create the project."));
        return nextState;
    }
