...
export function renderProjectView({ project, todoTasks, completedTasks }) {
    return `
        <div class="h-full flex flex-col min-h-0">
            <header class="px-10 py-8 flex justify-between items-start gap-6 flex-shrink-0 z-10 bg-white">
                <div class="flex flex-col gap-3 max-w-3xl">
                    ...
                    <h1 class="text-4xl font-medium tracking-tighter lowercase">${escapeHtml(project.name)}</h1>
            ...
            <div class="px-10 pb-6 flex-shrink-0 z-10 bg-white">
                <div class="grid grid-cols-1 xl:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)] gap-4">
                    ...
                    <div class="group flex items-center gap-3 bg-stone-50 rounded-2xl px-5 py-4 border border-stone-200/60 focus-within:border-stone-400 focus-within:bg-white focus-within:shadow-sm transition-all">
                        ...
                        <input id="taskInput" type="text" class="flex-1 bg-transparent border-none outline-none text-[15px] placeholder-stone-400 text-stone-900 lowercase" placeholder="add another task to this project...">
                    </div>
                </div>
            </div>
