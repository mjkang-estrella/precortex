export function renderNavigation({ currentView, inboxCount, navInboxCount, projectNav, projects, selectedProjectId, }) {
        ...
        return `
            ...
            : "w-full flex items-center justify-between gap-3 px-4 py-3 rounded-2xl text-sm transition-all text-stone-600 hover:text-stone-900 hover:bg-stone-50 lowercase group"}"
                ...
                >
                    ...
                    ${deadlineLabel
            ? `<span class="${isActive ? "text-white/70" : "text-stone-400"} text-[10px] font-semibold uppercase tracking-wider">${deadlineLabel}</span>`
            : ""}
                </button>
            `;
    })
